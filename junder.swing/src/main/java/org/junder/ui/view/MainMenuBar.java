...
public final class MainMenuBar extends JMenuBar {
	...
	private final Observer docObserver = new Observer() {
		...
		public void update(Observable o, Object t) {
			if (ModelEvent.MatrixLoaded.equals(t)) {
				...
				backMenu.setEnabled(Document.class.cast(o)
						.getModel().getParent().isPresent());
			}
		}
	};
	...
	public MainMenuBar(Document doc) {
		...
		view.add(menuItem(
				(e)->{doc.makeColumnMatrix();},
				"Column Sub Matrix",
				KeyEvent.VK_C));
		view.add(backMenu = menuItem(
				(e)->{doc.gotoParentMatrix();},
				"Back to Parent Matrix",
				KeyEvent.VK_B));
		add(view);
		add(navigate);
	...
	private static JMenuItem navItem(Document doc, Direction dir, int keyEvent) {
		return menuItem((e)->{doc.select(dir);}, "Go " + dir.toString(), keyEvent);
	}
